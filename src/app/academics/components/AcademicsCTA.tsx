...
const quickLinks = [
  { icon: 'UserGroupIcon', label: 'About Us', href: '/about', color: 'bg-primary/10 text-primary' },
  { icon: 'CalendarIcon', label: 'Upcoming Events', href: '/homepage#events', color: 'bg-accent/10 text-accent' },
  { icon: 'MapPinIcon', label: 'Campus Facilities', href: '/about#facilities', color: 'bg-success/10 text-success' },
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-16">
          {quickLinks.map((link, i) => (
            <Link key={link.label} href={link.href}
              className="reveal-hidden flex items-center gap-3 p-4 rounded-2xl border border-border bg-background hover:border-primary/30 hover:shadow-soft transition-all duration-200 card-lift group"
              style={{ transitionDelay: `${0.08 * i}s` }}>
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${link.color}`}>
                <Icon name={link.icon as 'UserGroupIcon'} size={17} />
              </div>
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <p className="text-white/65 text-base sm:text-lg max-w-xl mx-auto leading-relaxed mb-8">
              Join Greenwood Academy's 2025–26 batch. Experience world-class academics, co-curriculars, and a nurturing environment that sets students up for life.
            </p>

            {/* Seats progress */}
            <div className="max-w-sm mx-auto mb-8 p-4 rounded-2xl text-left"
              style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', backdropFilter: 'blur(12px)' }}>
              <div className="flex items-center justify-between text-xs mb-2">
                <span className="text-white/70 font-medium">Seats Filling Fast</span>
                <span className="text-white font-bold">72% Filled</span>
              </div>
              <div className="h-2 rounded-full bg-white/20 overflow-hidden">
                <div className="h-full rounded-full" style={{ width: '72%', background: 'linear-gradient(90deg, #38BDF8, #818CF8)' }} />
              </div>
            </div>

            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/homepage#admissions"
