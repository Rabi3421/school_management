...
const heroStats = [
  { icon: 'AcademicCapIcon', label: '6 Programs', sub: 'Nursery – Grade XII', color: 'from-blue-500 to-cyan-400' },
  { icon: 'TrophyIcon', label: '100% Pass Rate', sub: '8 Consecutive Years', color: 'from-amber-500 to-orange-400' },
  { icon: 'UserGroupIcon', label: '120+ Faculty', sub: 'Across 6 Departments', color: 'from-violet-500 to-purple-400' },
  { icon: 'BeakerIcon', label: '12 Labs', sub: 'Science & Tech', color: 'from-emerald-500 to-teal-400' },
];
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-16 lg:py-24 w-full">
        ...
        <nav className="reveal-hidden flex items-center gap-2 mb-8 text-white/50 text-sm">
          {[{ label: 'Home', href: '/homepage' }, { label: 'Academics', href: '/academics' }].map((b, i, arr) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <div>
            <div className="reveal-hidden inline-flex items-center gap-2 px-4 py-2 rounded-full mb-6"
              ...
              <span className="text-white/90 text-xs font-semibold tracking-wide">CBSE Affiliated · Nursery to Grade XII</span>
            </div>
            ...
            <p className="reveal-hidden text-base sm:text-lg text-white/65 leading-relaxed mb-8 max-w-lg" style={{ transitionDelay: '0.2s' }}>
              From play-based early learning to competitive board preparation, Greenwood Academy's curriculum is designed to challenge, inspire, and equip every student for the future.
            </p>

            <div className="reveal-hidden flex flex-col sm:flex-row gap-3" style={{ transitionDelay: '0.3s' }}>
              <a href="#programs" className="flex items-center justify-center gap-2 px-7 py-3.5 text-sm font-semibold rounded-xl transition-all duration-200 group"
              ...
              <Link href="/homepage#admissions" className="flex items-center justify-center gap-2 px-7 py-3.5 bg-white text-primary text-sm font-semibold rounded-xl hover:bg-white/90 transition-all duration-200 shadow-soft">
            ...
            {heroStats.map((s, i) => (
              <div key={s.label}
                ...
                <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${s.color} flex items-center justify-center flex-shrink-0 shadow-md`}>
                  <Icon name={s.icon as 'AcademicCapIcon'} size={20} className="text-white" />
                </div>
                <div>
                  <div className="text-white font-bold text-base leading-tight">{s.label}</div>
