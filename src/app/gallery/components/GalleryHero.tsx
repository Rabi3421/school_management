...
export default function GalleryHero() {
  ...
  const floatingCards = [
    { icon: 'PhotoIcon', label: '500+', sub: 'Photos', color: 'from-blue-500 to-cyan-400', delay: '0s', pos: 'top-0 right-12' },
    { icon: 'FilmIcon', label: '30+', sub: 'Albums', color: 'from-violet-500 to-purple-400', delay: '0.8s', pos: 'bottom-0 right-12' },
    { icon: 'CalendarDaysIcon', label: '26', sub: 'Years', color: 'from-amber-500 to-orange-400', delay: '1.4s', pos: 'top-8 left-4' },
    { icon: 'UserGroupIcon', label: '2,400+', sub: 'Students', color: 'from-emerald-500 to-teal-400', delay: '2s', pos: 'bottom-8 left-4' },
  ];
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-16 lg:py-24 w-full">
        <nav className="reveal-hidden flex items-center gap-2 mb-8 text-white/50 text-sm">
          ...
          <span className="text-white/80 font-medium">Photo Gallery</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div>
            <div className="reveal-hidden inline-flex items-center gap-2 px-4 py-2 rounded-full mb-6" style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', backdropFilter: 'blur(12px)' }}>
              ...
              <span className="text-white/90 text-xs font-semibold tracking-wide">500+ Photos · 30+ Albums · 26 Years</span>
            </div>
            ...
            <p className="reveal-hidden text-base sm:text-lg text-white/65 leading-relaxed mb-8 max-w-lg" style={{ transitionDelay: '0.2s' }}>
              From sports days and science fairs to cultural nights and graduation ceremonies — every moment at Greenwood is worth remembering.
            </p>
            <div className="reveal-hidden flex flex-col sm:flex-row gap-3" style={{ transitionDelay: '0.3s' }}>
              <a href="#gallery-grid" className="flex items-center justify-center gap-2 px-7 py-3.5 text-sm font-semibold rounded-xl transition-all duration-200 group" style={{ background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.25)', backdropFilter: 'blur(12px)', color: 'white' }}>
                Browse Gallery
          ...
          <div className="relative hidden lg:flex items-center justify-center h-80">
            <div className="absolute z-20 w-36 h-36 rounded-full flex flex-col items-center justify-center text-center shadow-2xl" style={{ background: 'rgba(255,255,255,0.12)', border: '2px solid rgba(255,255,255,0.25)', backdropFilter: 'blur(16px)' }}>
              <Icon name="PhotoIcon" size={32} className="text-white mb-1" />
              <div className="text-white font-bold text-sm leading-tight">Our<br />Memories</div>
            </div>
            {floatingCards.map((card) => (
              <div key={card.label} className={`absolute ${card.pos} flex items-center gap-3 px-4 py-3 rounded-2xl shadow-xl animate-float`} style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', backdropFilter: 'blur(16px)', animationDelay: card.delay }}>
                <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${card.color} flex items-center justify-center flex-shrink-0`}>
                  <Icon name={card.icon as 'PhotoIcon'} size={16} className="text-white" />
                </div>
                <div>
